...
suite(`UpdateNotebookMetadata`, () => {
    ...
    test('No Change when selecting remote kernel spec with matching kernelspec', async () => {
        // Regression test: selecting a remote kernel spec (e.g. from jupyterServerProvider)
        // on a notebook that already has a matching kernelspec should NOT mark the notebook dirty.
        const notebookMetadata: nbformat.INotebookMetadata = {
            orig_nbformat: 4,
            kernelspec: { display_name: 'My Remote Python', language: 'python', name: 'myremotepython' },
            language_info: { name: 'python' }
        };
        const remoteKernelSpec: IJupyterKernelSpec = {
            argv: ['python', '-f', '{connection_file}'],
            display_name: 'My Remote Python',
            name: 'myremotepython',
            language: 'python',
            executable: 'python'
        };
        const kernelConnection = RemoteKernelSpecConnectionMetadata.create({
            id: 'remote-spec-connection-id',
            baseUrl: 'http://remoteserver:8888',
            serverProviderHandle: { id: 'someProvider', handle: 'someHandle', extensionId: 'someExtension' },
